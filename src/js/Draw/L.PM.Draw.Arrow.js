...
Draw.Arrow = Draw.Line.extend({
    ...
    enable(options) {
        ...
        var arrowSymbol = L.Symbol.arrowHead({pixelSize: 15, polygon: false, pathOptions: arrowHintOptions});
        this._hintArrow = L.polylineDecorator(this._hintline, {
            patterns: [{offset: '100%', repeat: 0, symbol: arrowSymbol}]
        });
        this._layerGroup.addLayer(this._hintArrow);
    ...
    _syncHintLine() {
        ...
        if(polyPoints.length > 0) {
            ...
            this._hintArrow.setPaths([lastPolygonPoint, this._hintMarker.getLatLng()]);
        }
    }
});